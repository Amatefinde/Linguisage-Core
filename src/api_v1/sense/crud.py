...
async def check_is_user_have_sense_by_f_sense_id(session: AsyncSession, user: User, f_sense_id: int) -> bool:
...
async def add_personalize_sense_to_user(
    ...
    db_sense_images = [SenseImage(f_img_id=img.f_img_id) for img in personalize_sense_entity.sense_images]
    db_sense = Sense(
...
async def get_senses(
    ...
    page: int | None = None,
    limit: int | None = None,
    status: list[sense_status_type] | None = None,
) -> list[SGetSense]:
    stmt = (
        select(Sense)
    ...
    if status is not None:
        stmt = stmt.where(Sense.status.in_(status))
...
def __mark_already_added_senses(dictionary_word_info: DictionaryWordInfo, added_senses: Sequence[Sense]):
...
async def mark_senses_that_user_already_have(
    ...
    stmt = select(Sense).where(Sense.user == user).where(Sense.f_sense_id.in_(f_sense_ids_for_word))
    row_response = await session.execute(stmt)
