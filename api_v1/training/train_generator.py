...
async def _pick_senses_for_mastering_from_queue(
    ...
    f_sense_id_in_queue: list[SenseDTO] = await get_senses_for_user(session, user, status="in_queue")
...
async def _get_senses_for_training(session: AsyncSession, user: User, number: int) -> list[SenseDTO]:
    ...
    if (senses_not_enough := number - len(senses_in_process)) > 0:
        senses_from_queue = await _pick_senses_for_mastering_from_queue(session, user, senses_not_enough)
...
async def generate(session: AsyncSession, user: User, number: int) -> list[dictionary_provider.SSenseP]:
    senses_for_training: list[SenseDTO] = await _get_senses_for_training(session, user, number)
    if not senses_for_training:
        return []
